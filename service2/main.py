...
def callback(ch, method, properties, body):
    with tracer.start_as_current_span("service2-process-message"):
        data = json.loads(body)
        logger.info(f"[service2] Received message: {data}")
...
def start():
    ...
    while True:
        try:
            ...
            channel.queue_declare(queue=settings.queue_name_to_second_service, durable=True)
            channel.queue_bind(
                exchange=settings.exchanger,
                queue=settings.queue_name_to_second_service,
                routing_key=settings.routing_key_to_second_service,
            )

            channel.basic_consume(queue=settings.queue_name_to_second_service, on_message_callback=callback, auto_ack=True)
            logger.info("[service2] Waiting for messages...")
