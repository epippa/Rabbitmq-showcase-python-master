...
jaeger_exporter = JaegerExporter(
    agent_host_name="template_jaeger",
    agent_port=6831,
)
...
async def start():
    while True:
        try:
            print("[service1] Connecting to RabbitMQ...")
            # Usa host e porta definiti nelle settings/env:
            connection = await aio_pika.connect_robust(
                f"amqp://guest:guest@{settings.rabbitmq_host}:{settings.rabbitmq_port}/"
            )
            print("[service1] Connected to RabbitMQ")
            ...
            print(f"[service1] Waiting for messages on queue '{settings.queue_name_to_first_service}'...")
            async with queue.iterator() as queue_iter:
                async for message in queue_iter:
                    await publish_to_queue(message)
        except AMQPConnectionError as e:
            print(f"[service1] AMQP Connection failed: {e}. Retry in 2s...")
            await asyncio.sleep(2)
        except Exception as e:
            print(f"[service1] Unexpected error: {e}. Retry in 2s...")
            await asyncio.sleep(2)
